	...
	public static void main(String[] args) {
		...
		/*
		 		자료형 : 자바는 변수명 앞에 반드시 자료형이 선언되어야 한다.
		 			- 기본 자료형 (8개) : boolean(참거짓), 정수(byte, short, int, long), 실수(float, double), 문자(char)
		 					- 변수명과 값이 Stack 메모리 영역에 저장됨
		 					- 사용전에 반드시 초기값이 할당되어 있어야 한다.
		 					- 첫자가 소문자로 되어있다.
		 					- 		==   <-------- Stack 영역의 값을 비교하는 연산자.
		 					
		 			- 참조 자료형 : 배열 / 열거 / 객체(class) / 인터페이스
		 			 		- 무한대로 만들수 있다.
		 			 		- 변수명 (값이 저장되어 있는 heap영역의 주소가 Stack에 저장) : Heap 영역의 참조 주소 값이 변수명에 저장되어 있다.
		 			 		- 값 (Heap영역에 저장되어있음)
		 			 		- 참조 자료형에서 ==  <--------- Stack 의 값을 비교(Heap의 주소를 비교)
		 			 										따라서 값을 비교할때는 Heap에 저장되어 있는 값을 비교해야하므로 equals()를 사용해야한다.
		 			 		- Heap 영역은 반드시 초기값이 들어있어야한다.
		 			 		- 변수 선언만 하고 초기값을 넣지 않더라도 강제로 자동으로 할당이 됨.
		 			 		- boolean (초기값 - false), 정수[byte, short, int, long](초기값 - 0), 실수[float, double](초기값 - 0.0)
		...
		//1. 변수 선언 : 하나의 값만 저장됨. 변수에 다른 값을 저장할수 있다.
			int a = 10;
		...
		//2. 배열 선언
			int[] arr1 = new int[3];	//선언을 먼저하고 나중에 뒤에껄 써도 됨.
			// int[] : 배열 변수에 저장할 자료형, arr1 : 배열 변수명,
			// new : 변수의 값을 Heap 영역에 저장하겠다.
			// int[3] : 배열의 방의 갯수 : 3 ------------- 방번호는 0, 1, 2 번방이 생긴다.(index(0,1,2))라고함
		...
		//3. 배열의 기본값을 출력 : 선언만 해도 강제로 초기값이 Heap에 들어감.
			...
			System.out.println(arr1[2]);  // arr1 배열변수의 2번방의 값을 출력//3번방을 출력하면 오류남.
		...
		//4. 배열 방의 값을 저장
			...
			arr2 = new double[6];	// 선언 후 나중에 방크기 지정, index(방번호) : 0,1,2,3,4,5
			
			System.out.println(arr2[0]); 		//0.0 기본 초기값이 자동으로 저장되어 있음.
			...
			arr2[4] =50.0;
			arr2[5] =60.0;
		//	arr2[6] =60.0; 			// ArrayIndexOutOfBoundsException (배열의 방번호를 알수없다.)
			...
			for (int i = 0 ; i <100 ; i++) {
				arr3[i] = i+1;			//배열의 각방에 1,2,3,4,5..... 을 넣는것
